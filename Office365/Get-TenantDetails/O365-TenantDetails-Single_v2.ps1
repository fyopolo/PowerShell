...
$AllLicenses = $LicInit | Where-Object {$_.TargetClass -eq "User" -and $_.AccountSkuId -notcontains "*WINDOWS_STORE*"} | Select AccountSkuId, SkuPartNumber, TargetClass, ActiveUnits, WarningUnits, ConsumedUnits
...
$rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(0) -Tabheading (" ")
    $rpt+= Get-HtmlContentOpen -HeaderText "Licenses Info"
        ...
        $rpt+= Get-HtmlContentOpen -HeaderText "Domains in Tenant"
            $rpt+= Get-HtmlContentTable ($TenantDomains | Select Name, Status, IsDefault, IsInitial, Capabilities | Sort Name) -Fixed
        $rpt+= Get-HtmlContentClose
...
$rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(1) -Tabheading (" ")
    ...
    $rpt += Get-HtmlContentOpen -HeaderText "Mailboxes"
        $rpt+= Get-HTMLContentDataTable -ArrayOfObjects ($Mailboxes | Sort 'Recipient Type Details','Display Name') -PagingOptions '25,50,100,200,500,' -HideFooter
...
$rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(6) -Tabheading (" ")
    ...
    ## SECTION DETAILS
    $rpt+= Get-HTMLContentDataTable -ArrayOfObjects ($OI | Sort 'Recipient Type Details') -PagingOptions '25,50,100,200,500,' -HideFooter
