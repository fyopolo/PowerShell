...
# $WinDevices = Get-MgDevice -DeviceId 0d0a9968-199b-46ec-a0b6-cd729f247d76
...
foreach ($Device in $WinDevices){
    ...
    foreach ($Item in $RetKeys){
        ...
        foreach ($Case in $BLKeyInfo){
            ...
            SWITCH ($Case.VolumeType){
                1 { [string]"$($VolType = "Operating System Drive")`r`n" }
                2 { [string]"$($VolType = "Fixed Data Drive")`r`n" }
            }

            [string]$DriveType += "$($VolType)`r`n"

            $BLKeyID += "$($Case.Id)`r`n"
            $BLRKeyPwd += "$($Case.Key)`r`n"
        }
    ...
    $Hash =  [ordered]@{
        ...
        BitLockerKeyID    = $BLKeyID
        DriveType         = $DriveType
        RecoveryPassword  = $BLRKeyPwd
    }
...
# $Devices | Export-Excel -Path D:\Scripts\PowerShell\BitLocker\AAD-BLKeys_v2.xlsx -AutoSize -FreezeTopRow -BoldTopRow -AutoFilter
