...
foreach ($Account in Get-ADGroupMember -Identity "GSG_LF_Users")
{

    # Write-Host $Account.SamAccountName #// Uncomment this if you need to see/troubleshoot variable values
    New-Item D:\Scans\$($Account.SamAccountName) -ItemType Directory -Force
    $Subfolder=($Account.SamAccountName)
    $NTDOM="DRMC\"+$Subfolder
    Add-NTFSAccess -Path D:\Scans\$Subfolder -Account $NTDOM -AccessRights FullControl
