...
# Connect-MSGraph needs module to be installed. Use this: Install-Module -Name Microsoft.Graph.Intune
...
$allScripts | ForEach-Object {$PSItem.scriptContent | Out-File -FilePath $(Join-Path -Path $PSScriptRoot -ChildPath "$($psitem.displayName).ps1")}
# export all scripts and metadata within a csv file  to current directory
$allScripts | Export-Csv -Path $(Join-Path -Path $PSScriptRoot -ChildPath "Intune-PowerShellScripts.csv") -Delimiter ";" -NoTypeInformation