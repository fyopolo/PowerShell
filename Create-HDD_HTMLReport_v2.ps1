    ...
    Process {
        ...
        #             foreach ($computer in $computername) {
            ...
            Try {
                ...
                #             $cs = New-CimSession -ComputerName $computer -ErrorAction SilentlyContinue
                ...
                IF ($pscmdlet.ShouldProcess($env:Computername, "Get Disk Information")) {
                    ...
                    $drives = foreach ($item in $data) {
                        ...
                        $prophash = [ordered]@{
                            ...
                            FreeGB        = [math]::Round(($item.Freespace / 1GB),2)
                    ...
                    $pop.value = (Get-Ciminstance -ClassName Win32_OperatingSystem -Property caption).caption
...
New-HTMLDiskReport -Computername localhost -Path C:\TEMP\HDDreport.htm -Precontent "<h3>Company Confidential</h3>" -PostContent "This report is offered as-is. You can verify results with a command like <b>Get-Volume</b>." -logopath D:\DOCS\Companies\TeamLogicIT\Designs\NewYear\TLIT-LOGO.jfif
Invoke-Expression C:\TEMP\HDDreport.htmn