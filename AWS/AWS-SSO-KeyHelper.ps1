...
Param(
    ...
    [Parameter(Mandatory=$false,HelpMessage="Enter in ADFS Server")][string]$ADFSServer
    )


# AWS Account ID mappings Variables - Add any mapping here otherwise the account ID will be displayed.
# (File format: each line contains account number and name: "12345678912=Account Name")
...
#Check for a realm parameter and extract it
if ($accountmap['realm']) {
...
if($type -ne 'cli'){
    ...
    ##call AWS sts to assume the role and get access keys and token
