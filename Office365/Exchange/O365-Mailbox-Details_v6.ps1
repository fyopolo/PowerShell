...
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Importing Modules" -PercentComplete ($Step / 6 * 100)
...
### Progress Bar
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Working on Mailboxes" -PercentComplete ($Step / 6 * 100)
...
### Query
    ...
    ## REPORT: CHART ELEMENTS // Chart needs a COUNT column (defined in the SELECT statement)
    
    $MailboxesRPT = @()

    Foreach ($Mailbox in $Mailboxes)
    {
        $MailboxesRPT += Get-MailboxStatistics -Identity ($Mailbox.'Primary SMTP Address') | Select-Object @{name="Name";expression={$_.DisplayName}}, @{name="Count";expression={[math]::Round((($_.TotalItemSize.Value.ToString()).Split("(")[1].Split(" ")[0].Replace(",","")/1MB),2)}}
    }
    ...
    $rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(0) -Tabheading ("$Mailboxes")
        ...
        $rpt += Get-HTMLBarChart -ChartObject $BarObject -DataSet ($MailboxesRPT | Sort-Object count -Descending | Select -First 5)
...
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Working on Distribution Lists" -PercentComplete ($Step / 6 * 100)
...
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Working on Office 365 Groups" -PercentComplete ($Step / 6 * 100)
...
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Working on Mail Contacts" -PercentComplete ($Step / 6 * 100)
...
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Working on Extended Permissions" -PercentComplete ($Step / 6 * 100)
...
$Step++
Write-Progress -Id 1 -Activity $Task -Status "Step $Step of 6" -CurrentOperation "Looking for other recipient types" -PercentComplete ($Step / 6 * 100)
