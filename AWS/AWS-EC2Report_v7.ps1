...
# $LocalUserName = ((Get-CimInstance -ClassName Win32_ComputerSystem).Username).Split("\")[1]
# IF (Test-Path C:\Users\$LocalUserName\.aws) { $Connection = "AWSCLI" }

Import-Module AWSPowerShell

$AWSLICProfiles = Get-AWSCredential -ListProfileDetail | Sort ProfileName
IF ($AWSLICProfiles.Count -gt 0) {

    $LocalProfiles = @()
    $IndexFlag = 0

    foreach ($Prof in $AWSLICProfiles){

        $IndexFlag ++
        $HashProfiles = [ordered]@{
            MenuOption      = $IndexFlag
            ProfileName     = $Prof.ProfileName
            StoreTypeName   = $Prof.StoreTypeName
            ProfileLocation = $Prof.ProfileLocation
        
        }

    # Creating custom PS object out of Hash Table

    $NewOBJ = New-Object psobject -Property $HashProfiles
    $LocalProfiles += $NewOBJ
...
Write-Host ""
Write-Host "$($LocalProfiles.Count) AWS CLI local profile(s) was/were found" -ForegroundColor Cyan
Write-Host ""

#   Presenting Menu
Write-Host "Chose a Profile option to connect" -ForegroundColor Cyan
$LocalProfiles| ft -AutoSize -Wrap # -HideTableHeaders

#   Validation Script Block.
Do {
    Try {
        $numOk = $true
        [int]$Menu = Read-host "Please type a Menu Option. Type 0 for cancel and try STS Keys"
        } # End Try
    
    Catch {
            $numOK = $false
            Write-Host "The value you've entered is out of scope. Please try again." -ForegroundColor Red
          } # End Catch

        IF ($Menu -lt 1 -and $IndexFlag -gt 4)
            {
                Write-Host "The value you've entered is out of scope. Please try again." -ForegroundColor Red
            } #End IF
...
Until (($Menu -ge 1 -and $Menu -le $IndexFlag) -and $numOK -eq $true -or $Menu -eq 0)
...
SWITCH ($Menu){
    ...
    0 {
        Write-Host ""
        Write-Host "The option you've chosen was: $Menu. Let's try using STS values." -ForegroundColor Green
        [string]$AccessKey = Read-Host "Enter Access Key"
        [string]$SecretAccessKey = Read-Host "Enter Secret Access Key"
        [string]$SessionToken = Read-Host "Enter Session Token"
        Write-Host ""
        Write-Host "Getting EC2 Instances..." -ForegroundColor Cyan        
        $EC2 = (Get-EC2Instance -AccessKey $AccessKey -SecretKey $SecretAccessKey -SessionToken $SessionToken).Instances
        Write-Host ""
    ...
    default {
        $LocalProfName = $($LocalProfiles.GetValue($Menu -1).ProfileName)
        $Creds = Get-AWSCredential -ProfileName $LocalProfName
        $EC2Instances = @()
        Write-Host ""
        Write-Host "The option you've chosen was: $Menu" -ForegroundColor Green
        Write-Host "Connecting using local $LocalProfName profile" -ForegroundColor Green
        Write-Host ""
        Write-Host "Getting EC2 Instances..." -ForegroundColor Cyan
        $AAA = Get-AWSRegion | ForEach-Object {
            $EC2ByRegion = (Get-EC2Instance -Credential $Creds -Region $_.Region).Instances
                $HashEC2ByRegion = [ordered]@{
                Region     = 
 ...
 #   $AAA.region
    ...
    $NewOBJ = New-Object psobject -Property $HashEC2ByRegion
    $EC2Instances += $NewOBJ
            
            }
        # $EC2 = (Get-EC2Instance -Credential $Creds -Region $_).Instances
#        Write-Host ""
#        Write-Host "Getting Volumes..." -ForegroundColor Cyan
#        Write-Host ""
...
Write-Host "Processing $($EC2.Count) EC2 Instances..." -BackgroundColor Blue
...
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleEC2RUN -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $SummaryTable.EC2Instances  -WorksheetName "Summary" -Numberformat '#,##0' -Append

# Stopped EC2 Detail
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleEC2STOP -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $StoppedEC2Count -WorksheetName "Summary" -Numberformat '#,##0' -Append
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleEC2STOPSize -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $StoppedEC2Storage -WorksheetName "Summary" -Numberformat '#,##0' -Append

# Snapshots Detail
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleSSTD -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $SummaryTable.SnapshotSTDCount -WorksheetName "Summary" -Numberformat '#,##0' -Append
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleSSTDSize -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $SummaryTable.SnapshotSTDSizeGB -WorksheetName "Summary" -Numberformat '#,##0' -Append
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleSARCH -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $SummaryTable.SnapshotARCHCount  -WorksheetName "Summary" -Numberformat '#,##0' -Append
Export-Excel -Path $FilePath -StartColumn 1 -InputObject $TitleSARCHSize -WorksheetName "Summary" -Append
Export-Excel -Path $FilePath -StartColumn 2 -InputObject $SummaryTable.SnapshotARCHSizeGB  -WorksheetName "Summary" -Numberformat '#,##0' -Append
...
Export-Excel -Path $FilePath -Title "EC2 Instances" -TitleSize 24 -TitleBold -WorksheetName "EC2 Instances" -Append -InputObject $EC2Instances
