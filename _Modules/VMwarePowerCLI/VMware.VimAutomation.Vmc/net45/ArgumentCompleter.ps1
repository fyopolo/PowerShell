function SetArgumentCompleters(){
    $cmdletParametersToComplete = @{
        "Set-VmcClusterEdrsPolicy" = @{
            "PolicyType" = { "cost", "performance", "rapid-scaleup", "storage-scaleup" }
        }
    }

    foreach($cmdlet in $cmdletParametersToComplete.Keys) {
        for($i = 0; $i -lt $cmdletParametersToComplete[$cmdlet].count; $i++){

            $parameterName = @($cmdletParametersToComplete[$cmdlet].Keys)[$i]
            $possibleValues = $cmdletParametersToComplete[$cmdlet][$parameterName]

            Register-ArgumentCompleter -CommandName $cmdlet -ParameterName $parameterName -ScriptBlock $possibleValues
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBqKYM+Gr3y3039
# wmhNkeLBCCvTOy6HJeeh/bWNm7XqpqCCD8swggTMMIIDtKADAgECAhBdqtQcwalQ
...
# HDAaoRiAFmh0dHA6Ly93d3cudm13YXJlLmNvbS8wLwYJKoZIhvcNAQkEMSIEIKBg
# 1sokvxSiKW47CmqA1+GAvctAsJGl7a51V9Hw4BhHMA0GCSqGSIb3DQEBAQUABIIB
# AKSnLJY361TDK8MvvjM0ohBwz35YlcEs1rkcRz1/LYsU7TBBHeURVCk4JJR6rUzh
# Xp/aqWeoqRf05azX2a/EUhKOVv5A9qInQ0FYTrTq8AoLUUiQDPaAal9C9KZA1NKF
# C3QIcflSjlS6ftoj+zPHTcfqusjt6rFNGEGa8fff4wqRERs4jcCFGFp2aZQbDnFZ
# LyQVSxiMWr7TTWEOeEAd1Z4F723W+eEQCVA5Xo69jQkNqvS2dxQMZN8T63NlP6Wc
# XR1spKHWtNOF64g11ifMpUFpvWW3mptZSdxcvkzmtpPZ1BP08DPxeKbh+CZyzB2y
# i4XJvpCXxBiP6W/iz+O4LS+hgg7IMIIOxAYKKwYBBAGCNwMDATGCDrQwgg6wBgkq
# hkiG9w0BBwKggg6hMIIOnQIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJ
# EAEEoGgEZjBkAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgUfZFiegB
# 9xhGbbHzydHRKzdJfQy2zNjO/v/dJTTqPHACEGAYxe8XCGh1mWEIhkU/apAYDzIw
# MjAxMDA4MjEzNDQ2WqCCC7swggaCMIIFaqADAgECAhAEzT+FaK52xhuw/nFgzKdt
...
# BgkqhkiG9w0BCQUxDxcNMjAxMDA4MjEzNDQ2WjArBgsqhkiG9w0BCRACDDEcMBow
# GDAWBBQDJb1QXtqWMC3CL0+gHkwovig0xTAvBgkqhkiG9w0BCQQxIgQgzNB8q/8S
# b1XOTRwWED2o7uZOSRYfF8J4Bxd3iljtu5wwDQYJKoZIhvcNAQEBBQAEggEA2q4+
# hBgiA7wY0+1s0SbKBKjgN25ZewKmQAX6K9dsyLlUycn47kHtpZcibbEHsIaElDY3
# Lnc0tl+293DO/lyG7vkUjgtUXj8Yj5aXs32zs82TiNFq1d5kzVDshDhlSAOdmumX
# GfWQv7/fD2Z6EznSSpb0iXQaVLk/N8NyEvn+8X8G31LsHmKDmEHyn/+UpgSKgY7s
# x8dIuXAwug2KP4LHzXUY2u/yYeaHm0nqyyQhzk6MujXX/Dw9BsH0aueTK7GSMobO
# gwJK5mNZ0qYZGqyLv+fcYIR+nbSuG0iGndZx4D43QLU9Rf/JJb+/rRZLWEXGw/EP
# xF257YooOh4poFGJMw==
