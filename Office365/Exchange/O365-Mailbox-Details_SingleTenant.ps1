# -------------------------------------------------------------------------------
# Script: O365-Tenant-Details_Single.ps1
...
# $TenantDetails.VerifiedDomains | ft -AutoSize
...
# $SplitSeparatorSmtpPrimary = "SMTP:"
# $SplitSeparatorSmtpAlias = "smtp:"
# $SplitOption = [System.StringSplitOptions]::RemoveEmptyEntries

#    Import: SkuID List file
$SkuList = Import-Excel -Path $(Get-FileName)
...
$UserMailboxes = Get-Mailbox -ResultSize unlimited | Where-Object {$_.SKUAssigned -eq "True"}
$Step++
...
foreach ($MBX in $UserMailboxes) {
    $Counter++
    Write-Progress -Id 1 -ParentId 0 -Activity "Getting mailboxes details" -Status "Processing $($Counter) of $($UserMailboxes.Count)" -CurrentOperation $MBX.DisplayName -PercentComplete (($Counter/$UserMailboxes.Count) * 100)
    ...
    $Hash =  [ordered]@{
        ...
        'Recipient Type'           =    $MBX.RecipientType
        ...
        'Mailbox Quota'            =    "$(($MBX.ProhibitSendReceiveQuota.Substring(0,3)).Trim()) GB"
...
$AccountName = $AllLicenses.AccountName.Item(0).ToString() | Select-Object -First 1
...
foreach ($Item in $AllLicenses){
    $Counter++
    $Sku1 = $Item.AccountSkuId.Replace($AccountName+":","")
    $CO = "$(($SkuList | Where-Object {$_.SkuId -eq $Sku1}).Name) `r"
...
foreach ($Item in $LicensedUsers) {
    ...
    $Sku = ($Item.Licenses.AccountSkuId).TrimStart($AccountName+":")
    ...
    $Hash =  [ordered]@{
        ...
        'AD Synced'           = ($UserMailboxes.UserPrincipalName -match $Item.UserPrincipalName).IsSynchronized
...
$TenantLic = @()
foreach($Item in $AllLicenses){
    IF ($Item.AccountSkuId.Count -lt 1000){
        $SkuID = ($Item.AccountSkuId.Replace($AccountName,"")).Replace(":","")

        $Hash =  [ordered]@{
            'Account Sku Name'  = ($SkuList | Where-Object {$_.SkuID -contains $SkuID}).Name
            'Active Units'      = $Item.ActiveUnits
            'Purchased Units'   = IF ($Item.ActiveUnits -le 1000){$Item.ActiveUnits} ELSE {"Free"}
...
[int]$LicPurchasedTOT = (($TenantLic | Where-Object {$_.'Purchased Units' -ne "Free"}).'Active Units' | Measure-Object -Sum).Sum
...
[int]$FreeTOT = $LicPurchasedTOT - $LicConsumedTOT
IF ($FreeTOT -le 0) { [string]$LicFreeTOT = "All purchased licenses are assigned" }
ELSE { [int]$LicFreeTOT = $FreeTOT }
...
$MailContacts = Get-Recipient -ResultSize unlimited | Where-Object {$_.RecipientType -eq "MailContact"}
$Step++
...
$ExtendedPermissions = Get-RecipientPermission | Where-Object {$_.Trustee -ne "NT AUTHORITY\SELF" -and $_.Trustee -ne "NULL SID"}
$Step++
...
foreach ($Other in $OtherIdentities) {
    ...
    $OIAliases = $null
    foreach($O in $Other){ $OIAliases += "$(($O.EmailAddresses).Substring(5)), `r" }
...
$rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(0) -Tabheading (" ")
    $rpt+= Get-HtmlContentOpen -HeaderText "Licenses Info"
        ...
        $rpt+= Get-HtmlContentOpen  -HeaderText "Summary"
            ...
            $rpt += Get-HtmlContenttext -Heading "Unassigned Licenses" -Detail $LicFreeTOT
        ...
        $D1 = $($TenantLic | Where-Object {$_.'Purchased Units' -ne "Free"} | Select-Object @{n="Name";e={$_.'Account Sku Name'}}, @{n="Count";e={$_.'Purchased Units'}})
        $D2 = $($TenantLic | Where-Object {$_.'Purchased Units' -ne "Free"} | Select-Object @{n="Name";e={$_.'Account Sku Name'}}, @{n="Count";e={$_.'Consumed Units'}})
...
foreach ($Item in $DLs) {
    ...
    $rpt += Get-HtmlContentTable ($Item | Select-Object -Property @{n='AD Synced';e={$_.IsDirSynced}},
        ...
        # @{n='Aliases';e={$($_.EmailAddresses -cmatch "smtp:") -Replace("smtp:","")}},
        @{n='Owner';e={$_.ManagedBy}}) -Fixed

    $Aliases = $null
    foreach ($X in $Item){ [string]$Aliases += "$(($X.EmailAddresses).SubString(5))`r" }

    $Members = Get-DistributionGroupMember -Identity $Item.Identity
            
    ## DETAILS SECTION: DISTRIBUTION LIST MEMBERS
    $rpt += Get-HtmlContentOpen -HeaderText "Members of: $Item"
        $rpt += Get-HtmlContentTable ($Members | Select-Object DisplayName, Alias, PrimarySMTPAddress, RecipientTypeDetails | Sort-Object DisplayName) -Fixed
    $rpt += Get-HtmlContentClose
            # $members | select -First 1 | fl
$rpt += Get-HtmlContentClose
}
...
foreach ($Item in $O365Groups) {
$Counter++
$Members = Get-UnifiedGroupLinks –Identity $Item.Identity –LinkType Members

## GROUPS
$rpt += Get-HtmlContentOpen -HeaderText "Group Name: $($Item.DisplayName)" # -IsHidden
    $rpt += Get-HtmlContentTable ($Item | Select-Object Owner, DisplayName, PrimarySMTPAddress, RecipientTypeDetails) -Fixed
    ## MEMBERS
    $rpt += Get-HtmlContentOpen -HeaderText "Members of: $($Item.DisplayName)"
        $rpt += Get-HtmlContentTable ($Members | Select-Object Name, PrimarySMTPAddress, RecipientTypeDetails | Sort-Object Name) -Fixed
    $rpt += Get-HtmlContentClose
...
$rpt += Get-HTMLTabContentClose ## CLOSING TAB
$rpt += Get-HTMLClosePage -FooterText "Team Logic IT // Year $((Get-Date).Year)" ##    CLOSING HTML REPORT
