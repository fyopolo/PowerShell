...
## COST CENTER TAG: Customer Name

$MasterAccountId = "533012497661"
$Creds = Get-AWSCredential -ProfileName Master
$AccountId = (Get-STSCallerIdentity).Account
# $AccountId = @(Get-EC2SecurityGroup -GroupNames "default")[0].OwnerId # Default SG can't be deleted hence we can take AccountId from its properties.

Get-ORGAccountList -ProfileName Master | Where-Object {$_.Status -eq "ACTIVE" -and $_.Id -ne $MasterAccountId} | Select Name, Id | Sort Name | Out-GridView
$AWSAccounts = Get-ORGAccountList -ProfileName Master
$AWSAccounts

Write-Host ""
Write-Host "Getting EC2 Instances..." -ForegroundColor Cyan
$EC2 = Get-EC2Instance -Filter @( @{name='architecture'; values="x86_64"}) -Credential $Creds -Region us-east-1 | Select -ExpandProperty Instances
# $EC2.Count
Write-Host ""
...
$Snapshots = Get-EC2Snapshot -Credential $Creds -Region us-east-1 -Filter @{name='OwnerId'; values="$AccountId"}


$EC2.Count
$Snapshots.Count
# ($Snapshots | Where-Object {$_.OwnerId -eq "099720109477"}).Count

Get-ORGAccount 

Get-STSCallerIdentity -ProfileName Master
