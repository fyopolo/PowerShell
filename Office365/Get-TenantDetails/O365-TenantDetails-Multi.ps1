...
# Connect-ExchangeOnline -UserPrincipalName $credential.UserName -ShowProgress $true
...
foreach ($item in $Partners){

    Connect-AzureAD -TenantId $item.CustomerId
    Get-AzureADTenantDetail
    # Connect-ExchangeOnline -DelegatedOrganization $item.CustomerId -UserPrincipalName $credential.UserName -ShowProgress $true
...
$MsResponse = Get-MSALToken -Scopes @("https://graph.microsoft.com/.default") -ClientId "1b730954-1685-4b74-9bfd-dac224a7b894" -RedirectUri "urn:ietf:wg:oauth:2.0:oob" -Authority "https://login.microsoftonline.com/common/oauth2/nativeclient" -Interactive -ExtraQueryParameters @{claims='{"access_token" : {"amr": { "values": ["mfa"] }}}'}

# Get token for AAD Graph
$AadResponse = Get-MSALToken -Scopes @("https://graph.windows.net/.default") -ClientId "1b730954-1685-4b74-9bfd-dac224a7b894" -RedirectUri "urn:ietf:wg:oauth:2.0:oob" -Authority "https://login.microsoftonline.com/common/oauth2/nativeclient"
...
$PartnerTenants = Get-MsolPartnerContract -All | Select-Object Name, DefaultDomainName, TenantId

$PartnerTenants = Get-AzureADContract -All $True # | Select DisplayName, DefaultDomainName, ObjectId
...
foreach ($Tenant in $PartnerTenants){
    ...
    Connect-AzureAD -TenantId $Tenant.CustomerContextId
...
$AllLicenses = $LicInit | Where-Object {$_.TargetClass -eq "User" -and $_.AccountSkuId -notcontains "*WINDOWS_STORE*"} | Select-Object AccountSkuId, SkuPartNumber, TargetClass, ActiveUnits, WarningUnits, ConsumedUnits
...
foreach ($Item in $LicensedUsers) {
    ...
    foreach($Li in $Sku){
        IF (($Sku | Measure-Object).Count -ge 1){ $Licenses += "$(($SkuList | Where-Object {$_.SkuId -eq $Li}).Name) `r" -join " + " }
        ELSE { $Licenses = $(($SkuList | Where-Object {$_.SkuId -eq $Li}).Name) }
        }
...
$rpt += Get-HtmlOpenPage -TitleText "Office 365 Tenant Report: $TenantDefaultDomain" -LeftLogoString "https://ownakoa.com/wp-content/uploads/2016/09/TeamLogic-IT-Logo.png"
