...
#################
    ...
    $MailboxesRPT = $Mailboxes | Get-MailboxStatistics | Select-Object @{name="Name";expression={$_.DisplayName}}, @{name="Count";expression={[math]::Round((($_.TotalItemSize.Value.ToString()).Split("(")[1].Split(" ")[0].Replace(",","")/1MB),2)}} -First 5
    ...
    $rpt += Get-HTMLTabContentOpen -TabName $TABarray.Item(0) -Tabheading ("$Mailboxes")
        ...
        $rpt += Get-HTMLBarChart -ChartObject $BarObject -DataSet ($MailboxesRPT | Sort-Object count -Descending)
