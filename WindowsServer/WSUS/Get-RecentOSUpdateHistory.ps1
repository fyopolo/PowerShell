...
ForEach ($server in $servers) {
    Invoke-Command -ComputerName $server -ScriptBlock {
        Get-HotFix | Where-Object {$_.InstalledOn -gt ((Get-Date).AddDays($days))} | Select-Object -Property PSComputerName, Description, HotFixID, InstalledOn } | Format-Table -AutoSize #| Out-File -Encoding utf8 -FilePath $output -Append -ErrorAction SilentlyContinue
}