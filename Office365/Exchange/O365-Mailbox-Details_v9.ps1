...
# 05/04/2018  Feature Added: Scanning all customers' tenants (only for MSPs)
# -------------------------------------------------------------------------------
...
$A = Get-MsolPartnerContract | Get-MsolDomain | Where-Object {$_.IsDefault -eq 'True'}
$Count = -1
$reports = @()
$PBar = 0
...
foreach ($Tenant in $Customers)
{
    $Count++
    $PBar++
    # write-host "Customer:" $Tenant.Name
    # Write-Host "Default Domain:" ($A.Item($Count)).Name
    
    $reportHash =  [ordered] @{
        CustomerName    = $Tenant.Name
        DefaultDomain   = ($A.Item($Count)).Name
    }

    $reportObject = New-Object psobject -Property $reportHash
    $reports += $reportObject

    #   Enable below code ONLY if you are an MSP
    # Write-Progress -Id 1 -ParentId 0 -Activity "Getting Available Tenants" -Status "Working on $($Tenant.Name)" -PercentComplete (($PBar/$Customers.Count)*100)
    # Start-Sleep -Milliseconds 50
}
